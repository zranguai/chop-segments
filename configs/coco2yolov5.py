...
def coco2yolov5(coco_img_dir, coco_ann_file, output_dir, obj_names_file=None,
    ...
    # try:
    #     os.symlink(coco_img_dir, images_set_dir)  # 设置软连接
    ...
    for img_id, img in coco.imgs.items():
        ...
        ann_ids = coco.getAnnIds(imgIds=img_id, catIds=cat_ids)  # 根据img_id查找ann_ids
        if len(ann_ids) <= 0:
            if print_ignored:
        ...
        coco_copy_img = coco_img_dir + '/' + os.path.basename(img_path)
        copy(coco_copy_img,images_dir)
        yolo_txt.write(img_path)
        yolo_txt.write(os.linesep)

        anns = coco.loadAnns(ann_ids)  # 根据ann_ids加载coco标注信息
...
def parse_args(coco_img_dir="./cut_image/coco_cut4/train",
               coco_ann_file="./cut_image/coco_cut4/annotations/instances_train2017.json",
    ...
    parser = argparse.ArgumentParser(prog =current_path,usage="python scripts/coco2yolov5.py <options>")
...
def coco2yolo(coco_img_dir,coco_ann_file,output_dir):
    args = parse_args(coco_img_dir,coco_ann_file,output_dir)
    coco2yolov5(
...
if __name__ == "__main__":
    ...
    coco_img_dir = "./cut_image/cut_sdk3.0/coco_cut4/train"
    coco_ann_file = "./cut_image/cut_sdk3.0/coco_cut4/annotations/instances_train2017.json"
    output_dir = "./cut_image/cut_sdk3.0/txt_cut4"

    coco2yolo(coco_img_dir,coco_ann_file,output_dir)
