    ...
    def data_transfer(self):
        for num, json_file in enumerate(self.labelme_json):
            with open(json_file, 'r') as fp:
                # print("json_name=", json_file)  # print for debug
                data = json.load(fp)
...
def copyImage(imgdir,dst_dir='./cut_image/coco_cut2/train'):
    # imgdir = './image/'
    # imgdir = './dst_data/'
    # imgdir = '..//data//luhou/'
    # imgdir = './src_data/'
    for file in os.listdir(imgdir):
        if file.endswith('.jpg'):
            shutil.copyfile(imgdir + '/' +file, dst_dir+ '/' + file)
...
if "__main__" == __name__:
    ...
    # copyImage()
    # labelme2coco(labelme_json, './coco_cut1/annotations/train.json')
    # copyCategory()

    # labelme_json = glob.glob('./dst_data/*.json')
    labelme_json = glob.glob('..//data//luhou/*.json')  # json路径
    if not os.path.exists("coco_cut2/annotations/"):
        os.makedirs('coco_cut2/annotations/')
    if not os.path.exists('coco_cut2/train/'):
        os.makedirs('coco_cut2/train/')

    imgdir = '..//data//luhou/'  # 图片路径
    copyImage(imgdir)
    labelme2coco(labelme_json, 'coco_cut2/annotations/train_cut1.json')
    # copyCategory()
